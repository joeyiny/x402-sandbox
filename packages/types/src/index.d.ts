export declare const DEFAULT_PORTS: {
    readonly rpc: 8545;
    readonly facilitator: 3001;
    readonly server: 3002;
};
export declare const DEFAULT_LOCAL_FACILITATOR_URL: string;
export declare const DEFAULT_CDP_FACILITATOR_URL = "https://api.developer.coinbase.com/x402/facilitator";
export declare const DEFAULT_TEST_ACCOUNTS: readonly [{
    readonly address: "0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266";
    readonly privateKey: "0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80";
}, {
    readonly address: "0x70997970C51812dc3A010C7d01b50e0d17dc79C8";
    readonly privateKey: "0x59c6995e998f97a5a0044966f0945389dc9e86dae88c7a8412f4603b6b78690d";
}, {
    readonly address: "0x3C44CdDdB6a900fa2b585dd299e03d12FA4293BC";
    readonly privateKey: "0x5de4111afa1a4b94908f83103eb1f1706367c2e68ca870fc3fb9a804cdab365a";
}];
export declare const DEFAULT_FACILITATOR_PRIVATE_KEY: "0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80";
export declare const DEFAULT_MNEMONIC = "test test test test test test test test test test test junk";
export type BlockchainType = 'local' | 'anvil' | 'external';
export interface BlockchainConfig {
    type?: BlockchainType;
    url?: string;
}
export type FacilitatorType = 'local' | 'cdp';
export interface FacilitatorConfig {
    type?: FacilitatorType;
    privateKey?: string;
    url?: string;
}
export interface ServerConfig {
    port?: number;
    pricing?: Record<string, string>;
    paymentAddress?: string;
    facilitatorUrl?: string;
}
export interface TokenConfigOverrides {
    deploy?: boolean;
    address?: string;
}
export interface SandboxConfig {
    blockchain?: BlockchainConfig;
    facilitator?: FacilitatorConfig;
    server?: ServerConfig;
    token?: TokenConfigOverrides;
}
export interface TokenConfig {
    name: string;
    symbol: string;
    decimals: number;
    initialSupply: string;
}
export declare const DEFAULT_TOKEN_CONFIG: TokenConfig;
//# sourceMappingURL=index.d.ts.map